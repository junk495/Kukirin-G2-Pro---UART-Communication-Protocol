...
- **Circumference**: \( C = \pi \cdot 22.86 \approx 71.81 \, \text{cm} = 0.7181 \, \text{m} \).
...
\[
v \approx \frac{2550}{\text{Speed Raw Value}}
\]
Where:
- \( v \): Speed in km/h.
...
**Physical Basis**:
- The wheel speed (\( f \), in revolutions per second) is related to the pulse frequency (\( f_{\text{pulse}} \)) from the Hall sensors:
  \[
  f = \frac{f_{\text{pulse}}}{15}.
  \]
- The speed in m/s is:
  \[
  v_{\text{m/s}} = f \cdot 0.7181,
  \]
  and in km/h:
  \[
  v_{\text{km/h}} = v_{\text{m/s}} \cdot 3.6.
  \]
- The raw value is inversely proportional to the pulse frequency (\( f_{\text{pulse}} \)), likely representing a scaled time per pulse. The constant 2550 was derived empirically from the reference table and approximates the relationship:
  \[
  v_{\text{km/h}} \approx \frac{2550}{\text{Speed Raw Value}}.
  \]

**Example Calculations**:
- For a raw value of 0x0DAC (3500 decimal, idle state): \( v \approx \frac{2550}{3500} \approx 0.73 \, \text{km/h} \approx 0 \, \text{km/h} \) (consistent with idle state).
- For a raw value of 0x0030 (48 decimal, Open profile, Level 3): \( v \approx \frac{2550}{48} \approx 53.13 \, \text{km/h} \) (matches V-Max of 53 km/h).
...
## 7. Example Code: Checksum Calculation

Below is a Python example for calculating the CRC-16/MODBUS (TX) and CRC-8 (RX) checksums used in the protocol.

```python
def crc16_modbus(data: bytes) -> int:
    """Calculate CRC-16/MODBUS checksum for TX packets (first 18 bytes)."""
    crc = 0xFFFF
    for byte in data:
        crc ^= byte
        for _ in range(8):
...
def crc8(data: bytes) -> int:
    """Calculate CRC-8 checksum for RX packets (first 13 bytes)."""
    crc = 0x00
    polynomial = 0x31
    for byte in data:
        crc ^= byte
...
# Example usage
tx_packet = bytes([0x01, 0x14, 0x01, 0x02, 0x05, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0C, 0x19, 0x01, 0xA4, 0x01, 0x68])
rx_packet = bytes([0x02, 0x0E, 0x01, 0x00, 0xC0, 0x00, 0x00, 0x00, 0x0D, 0xAC, 0x00, 0x00, 0x6C])

print(f"TX CRC-16: 0x{crc16_modbus(tx_packet):04X}")
print(f"RX CRC-8: 0x{crc8(rx_packet):02X}")
```
